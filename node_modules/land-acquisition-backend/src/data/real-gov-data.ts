...
export interface StateGovData {
  id: string;               // Standardised state ID
  name: string;             // Official name per MHA
  code: string;             // ISO 3166-2 short code
  capital: string;          // State/UT capital
  type: 'STATE' | 'UT';
  populationCensus2011: number;      // Census 2011 actual figure
  areaKm2: number;                   // Geographical area in sq km
  districtCount: number;             // As per MHA 2024
  gsdpLakhCroreFY25: number;         // GSDP ₹ Lakh Crore, FY 2024-25 estimate (RBI)
  // DILRMP Progress — DoLR Annual Report 2023-24
  dilrmp: {
    rorComputerizedPct: number;        // % Records of Rights computerized
    cadastralMapDigitizedPct: number;  // % Cadastral maps digitized
    ulpinImplemented: boolean;         // Bhu-Aadhaar / ULPIN status
    sroComputerizedPct: number;        // % Sub-Registrar Offices computerized
  };
  // Land Use — Wasteland Atlas 2019 + Agricultural Statistics 2022-23
  landUseKm2: {
    agricultural: number;   // Net sown area (km²)
    forest: number;         // Forest cover (km²)
    wasteland: number;      // Wastelands (km²)
    builtUp: number;        // Built-up / urban area (km²)
    other: number;          // Water bodies, pastures, etc.
  };
...
// ─────────────────────────────────────────────────────────────
export const REAL_STATES_DATA: StateGovData[] = [
  // ── NORTH INDIA ──────────────────────────────────────────
  {
...
// ─────────────────────────────────────────────────────────────
export const RFCTLARR_PROVISIONS = {
...
// ─────────────────────────────────────────────────────────────
export const DILRMP_NATIONAL_SUMMARY = {
  reportYear: '2023-24',
...
// ─────────────────────────────────────────────────────────────
export const REAL_DISTRICTS: Record<string, string[]> = {
  UP: [
