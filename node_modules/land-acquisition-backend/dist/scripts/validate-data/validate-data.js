...
exports.runDataValidation = runDataValidation;
...
function runDataValidation() {
    ...
    const dataDir = candidateDirs.find(dir => fs_1.default.existsSync(path_1.default.join(dir, 'raw/lgd_states.csv'))) || path_1.default.resolve(process.cwd(), 'data');
    ...
    let warnings = 0;
    // 1. Validate States
    if (!fs_1.default.existsSync(statesPath)) {
    ...
    if (!fs_1.default.existsSync(districtsPath)) {
    ...
    if (fs_1.default.existsSync(autoMatchPath)) {
        const autoMatches = parseCsv(fs_1.default.readFileSync(autoMatchPath, 'utf8'));
        console.log(`\n🗺️ Auto-Matched District Geometries: ${autoMatches.length} records available.`);
    }
    if (fs_1.default.existsSync(reviewMatchPath)) {
        const reviewMatches = parseCsv(fs_1.default.readFileSync(reviewMatchPath, 'utf8'));
