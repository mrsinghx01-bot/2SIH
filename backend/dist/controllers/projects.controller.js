...
async function getProjectById(req, res) {
    ...
    const matchingKey = Object.keys(PROJECT_GIS_CONFIG).find(key => project.name.toLowerCase().includes(key.toLowerCase()));
    ...
    const parcels = rawParcels.map((p, pIdx) => {
        if (p.geojson) {
            try {
                ...
                return { ...p, center, polygon: coords.map((c) => [c[1], c[0]]) };
            }
